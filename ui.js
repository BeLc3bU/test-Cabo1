export class UI {
    ...
    showTestResults(resultado, modo, onRepasarFallos, onVolverMenu) {
        ...
        const esEvaluacion = modo && modo.startsWith('simulacro');

        if (esEvaluacion) {
            const totalPreguntas = resultado.totalPreguntas || (resultado.aciertos + resultado.fallos);
            const puntuacionRaw = resultado.aciertos - (resultado.fallos * 0.33);
            const nota = Math.max(0, (puntuacionRaw / totalPreguntas) * 100);
            const esApto = nota >= 50;
            const noContestadas = Math.max(0, totalPreguntas - (resultado.aciertos + resultado.fallos));
    ...
    resetTestUI(estadoActual) {
        ...
        this.elements.revisionFallosEl.classList.add('oculto');
        this.elements.veredictoEl.classList.add('oculto'); // Limpiar y ocultar veredicto anterior
        // Ocultar "Seguir más tarde" solo en modos de simulacros para fomentar que se completen en una sesión.
        const esModoExamen = estadoActual.modo.startsWith('simulacro');
